...
exports.up = (knex, Promise) =>
  Promise.all([
    knex.schema.createTable('service_tag_whitelist', t => {
      t.uuid('service').references('service.id');
...
exports.down = (knex, Promise) =>
  ...
  .then(() =>
    Promise.all([
      knex('role_permission').where({role: 'administrator', permission: 'get_service_tags'}).del(),
      knex('role_permission').where({role: 'administrator', permission: 'update_service_tags'}).del()
    ])
  )
  .then(() =>
    Promise.all([
